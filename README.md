...
- 提供 `tiangong-workspace` 命令行应用：可查看环境信息、执行文档生成工作流、触发 Tavily 联网检索。
- 集成 Tavily MCP 搜索，结合 LangChain 工作流，为报告、计划书、专利交底书、项目申报书等常见材料提供稳定的写作脚手架。
...
- `src/tiangong_ai_workspace/`：工作区 Python 包与 CLI 入口。
  - `cli.py`：Typer CLI，包含 `docs`、`research` 与 `mcp` 子命令。
  - `agents/`：基于 LangChain + LangGraph 的文档工作流。
  - `tooling/`：响应封装、工具注册表、OpenAI 模型工厂与 Tavily MCP 搜索客户端。
...
3. 扩展 `agents/` 内的 LangGraph 工作流或新增模板，满足更多写作场景。
4. 同步更新 `AGENTS.md` 与 `README.md`，确保文档与代码一致。
